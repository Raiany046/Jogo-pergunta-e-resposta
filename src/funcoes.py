...
def salvar_pontuacao(pontos):
    with open("pontuacao.txt", "w", encoding="utf-8") as arquivo:
        arquivo.write(str(pontos))

def carregar_pontuacao():
    try:
        with open("pontuacao.txt", "r", encoding="utf-8") as arquivo:
            return int(arquivo.read())
    except:
        return 0

def pontuar(pergunta, pontuacao):
    '''
    ...
    if pergunta.dificuldade == 'facil':
            ...
            pontuacao += 3
    return pontuacao # Retorna o novo valor para quem chamou
...
def mensagem_final(vidas):
    
    if vidas == 3:
        return "Perfeito! Você dominou esta categoria."

    elif vidas == 2:
        return "Muito bem! Só cometeu alguns erros."

    elif vidas == 1:
        return "Foi por pouco, mas conseguiu chegar ao fim."

    else:
        return "Precisa estudar mais essa categoria."
